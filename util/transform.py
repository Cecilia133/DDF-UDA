...
class Compose(object):
    ...
    def __call__(self, image, label, edge, region):
        for t in self.segtransform:
            image, label, edge, region = t(image, label, edge, region)
        return image, label, edge, region


class ToTensor(object):
    # Converts numpy.ndarray (H x W x C) to a torch.FloatTensor of shape (C x H x W).
    def __call__(self, image, label, edge, region):
        if not isinstance(image, np.ndarray) or not isinstance(label, np.ndarray) or not isinstance(edge, np.ndarray):
        ...
        if not isinstance(edge, torch.LongTensor):
            edge = edge.long()
        region = torch.from_numpy(region)
        if not isinstance(region, torch.LongTensor):
            region = region.long()
        return image, label, edge, region
...
class Resize(object):
    ...
    def __call__(self, image, label, edge, region):
        ...
        region = cv2.resize(region, self.size[::-1], interpolation=cv2.INTER_NEAREST)
        return image, label, edge, region
...
class RandScale(object):
    ...
    def __call__(self, image, label, edge, region):
        ...
        region = cv2.resize(region, None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_NEAREST)
        return image, label, edge, region
...
class Crop(object):
    ...
    def __call__(self, image, label, edge, region):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            edge = cv2.copyMakeBorder(edge, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=self.ignore_label)
            region = cv2.copyMakeBorder(region, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=self.ignore_label)
        h, w = label.shape
        ...
        region = region[h_off:h_off + self.crop_h, w_off:w_off + self.crop_w]
        return image, label, edge, region
...
class RandRotate(object):
    ...
    def __call__(self, image, label, edge, region):
        if random.random() < self.p:
            ...
            edge = cv2.warpAffine(edge, matrix, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=self.ignore_label)
            region = cv2.warpAffine(region, matrix, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=self.ignore_label)
        return image, label, edge, region
